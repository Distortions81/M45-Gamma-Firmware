...
| `multi_pool_enabled`, `pool_statuses` | boolean/array | Multi-pool mode and per-pool status. Each pool status includes `pool_id`, `label`, `role`, `host`, `port`, `connected`, `using_backup`, `disabled`, `note`, `share_percent`, `connected_seconds`, `response_ms`, `pool_difficulty`, `work_received`, `submitted`, `accepted`, `rejected`, `payout_status`, and `payout_percent_x100`. Runtime-disabled pools include a short `note` such as an unsupported version mask. |
