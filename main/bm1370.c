...
#define BM1370_RESULT_LENGTH 11
#define BM1370_RESULT_JOB_RESPONSE_FLAG 0x80
...
void BM1370_set_job_difficulty(double difficulty)
{
    if (!(difficulty > 0.0)) {
        return;
    }
...
{   
    if (!(nonce_percent > 0.0) || !(frequency > 0.0f) || asic_count == 0 || cores == 0) {
        ESP_LOGW(TAG, "Skipping nonce-space update: percent=%g frequency=%g asics=%u cores=%u",
    ...
    if (!(hcn_frac > 0.0)) {
        ESP_LOGW(TAG, "Skipping nonce-space update: computed HCN is not positive (%g)",
                 hcn_frac);
        return;
    }
...
float BM1370_send_hash_frequency(float target_freq)
{
    ...
    if (refdiv == 0 || postdiv1 == 0 || postdiv2 == 0 || !(frequency > 0.0f)) {
        ESP_LOGE(TAG, "No valid PLL parameters for target frequency %g MHz", target_freq);
        return 0.0f;
    }
...
uint8_t BM1370_init(void * pvParameters)
{
    ...
    if (chip_counter <= 0 || chip_counter > 256) {
        ESP_LOGE(TAG, "Invalid BM1370 chip count detected: %d", chip_counter);
        return 0;
    }
    ...
    if (address_interval <= 0) {
        ESP_LOGE(TAG, "Invalid BM1370 address interval for %d chip(s)", chip_counter);
        return 0;
    }
...
void BM1370_send_work(void * pvParameters, bm_job * next_bm_job)
{
    ...
    id = (id + 24) % 128;
    job.job_id = id;
    job.num_midstates = next_bm_job->num_midstates > 0 ? next_bm_job->num_midstates : 1;
...
task_result * BM1370_process_work(void * pvParameters)
{
    ...
    if (receive_work(asic_result, sizeof(asic_result), &result.timestamp_us) != ESP_OK) {
        return NULL;
    }
    
    if ((asic_result[10] & BM1370_RESULT_JOB_RESPONSE_FLAG) == 0) {
        if (address_interval <= 0) {
            ESP_LOGW(TAG, "Ignoring register result before BM1370 address map is initialized");
            return NULL;
        }
        const uint8_t register_address = asic_result[7];
        result.register_type = register_type_for_address(register_address);
        if (result.register_type == REGISTER_INVALID) {
            ESP_LOGW(TAG, "Unknown register read: %02x", register_address);
            return NULL;
        }
        result.asic_nr = asic_result[6] / address_interval;
        result.value = read_be32(&asic_result[2]);
        
        return &result;
    }

    if (address_interval <= 0) {
        ESP_LOGW(TAG, "Ignoring nonce result before BM1370 address map is initialized");
        return NULL;
    }

    const uint8_t raw_job_id = asic_result[7];
    uint8_t job_id = (raw_job_id & 0xf0) >> 1;
    uint32_t nonce_h = read_be32(&asic_result[2]);
