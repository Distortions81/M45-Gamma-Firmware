...
const UPDATE_WEB_FLASHER_URL='https://m45core.github.io/M45-Gamma-Firmware/';
const UPDATE_RELEASES_URL=`${UPDATE_WEB_FLASHER_URL}canonical-releases.json`,UPDATE_ASSET_PREFIX='esp-miner-factory-602-';
...
function releaseCompareVersion(v){return String(v||"").replace(/^cfg[0-9]+-/,"").replace(/-dirty$/,"").replace(/\+.*/,"")}
...
function releaseVersion(release){return release&&String(release.tag_name||release.version||release.name||'')}
function matchingFactoryAssets(release){const tag=releaseVersion(release),assets=Array.isArray(release&&release.assets)?release.assets:[];
const otaPath=release&&release.ota_path;if(otaPath){const name=String(otaPath).split('/').pop();return name&&name.endsWith('.bin')?[{name,path:otaPath}]:[]}
return assets.filter(a=>a&&a.name&&a.name.startsWith(`${UPDATE_ASSET_PREFIX}${tag}`)&&a.name.endsWith('.bin'))}
function factoryMirrorUrl(release,asset){return asset.path?new URL(asset.path,UPDATE_WEB_FLASHER_URL).href:`${UPDATE_WEB_FLASHER_URL}firmware/${encodeURIComponent(releaseVersion(release))}/${encodeURIComponent(asset.name)}`}
function renderLatestRelease(release){latestRelease=release;const tag=releaseVersion(release)||'unknown';
...
try{const r=await fetch(UPDATE_RELEASES_URL,{cache:'no-store'});if(!r.ok)throw new Error(`release manifest ${r.status}`);
const payload=await r.json(),releases=Array.isArray(payload)?payload:Array.isArray(payload&&payload.releases)?payload.releases:[];const latest=releases.find(x=>x&&!x.prerelease&&!x.draft);
...
const installable=latestRelease&&latestFactoryAsset&&!currentMatchesReleaseTag(releaseVersion(latestRelease));
...
const url=factoryMirrorUrl(latestRelease,latestFactoryAsset);try{setOtaMessage(`downloading ${releaseVersion(latestRelease)}...`);
